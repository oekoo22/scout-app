...
class APIService {
    ...
    private let baseURL = "http://192.168.178.187:8000"
    ...
    func processLocalPDF(pdfData: Data, fileName: String, completion: @escaping (Result<ScoutOrchestratorResponse, APIError>) -> Void) {
        guard let url = URL(string: "\(baseURL)/process-local-pdf") else {
            completion(.failure(.invalidURL))
            return
        }
